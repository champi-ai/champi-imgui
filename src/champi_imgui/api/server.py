...
def create_mcp_app(**overrides: Any) -> FastMCP:
    ...
    Pass manager instances via **overrides to inject test doubles:
        create_mcp_app(canvas_manager=mock_cm, event_queue=mock_eq)
    """
    _server_start_time = time.monotonic()

    canvas_manager = overrides.get("canvas_manager") or CanvasManager()
    event_queue = overrides.get("event_queue") or EventQueue()
    theme_manager = overrides.get("theme_manager") or ThemeManager()
    layout_manager = overrides.get("layout_manager") or LayoutManager()
    data_store = overrides.get("data_store") or DataStore()
    binding_manager = overrides.get("binding_manager") or BindingManager(data_store)
    animation_manager = overrides.get("animation_manager") or AnimationManager()
    notification_manager = (
        overrides.get("notification_manager") or NotificationManager()
    )
    template_manager = overrides.get("template_manager") or TemplateManager()
    ...
    # Expose managers for test injection (avoids monkeypatching module globals)
    mcp._canvas_manager = canvas_manager  # type: ignore[attr-defined]
    mcp._event_queue = event_queue  # type: ignore[attr-defined]
    mcp._theme_manager = theme_manager  # type: ignore[attr-defined]
    mcp._layout_manager = layout_manager  # type: ignore[attr-defined]
    mcp._data_store = data_store  # type: ignore[attr-defined]
    mcp._binding_manager = binding_manager  # type: ignore[attr-defined]
    mcp._animation_manager = animation_manager  # type: ignore[attr-defined]
    mcp._notification_manager = notification_manager  # type: ignore[attr-defined]
    mcp._template_manager = template_manager  # type: ignore[attr-defined]
    ...
    mcp._create_widget_in_canvas = _create_widget_in_canvas  # type: ignore[attr-defined]
    ...
    def clear_canvas(canvas_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Sent CLEAR_CANVAS command to '{canvas_id}' (seq={seq_num})"
                )
        ...
        try:
            ...
            if canvas._running:
                ...
                try:
                    ...
                    logger.info(
                        f"Sent SHUTDOWN command to '{canvas_id}' (seq={seq_num})"
                    )
                finally:
                    shm_manager.cleanup()
    ...
    def add_tab_item(
        ...
        try:
            ...
            if tab_bar is None:
                return {
                    "success": False,
                    "error": f"TabBarWidget '{tab_bar_id}' not found",
                }
    ...
    def show_notification(
        ...
        try:
            try:
                ...
                return {
                    "success": False,
                    "error": f"Invalid type '{type}'. Valid: {valid}",
                }
    ...
    @mcp.tool()
    def unbind_data(
        source_path: str, target_widget: str | None = None
    ) -> dict[str, Any]:
    ...
    def import_canvas_json(filepath: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not canvas:
                return {
                    "success": False,
                    "error": f"Failed to import from '{filepath}'",
                }
    ...
    def add_image(
        ...
        try:
            widget = ImageWidget(
                widget_id, file_path=file_path, width=width, height=height
            )
            return _create_widget_in_canvas(canvas_id, widget)
        except Exception as e:
    ...
    def drawing_add_llm_stroke(
        ...
        try:
            ...
            logger.error(
                f"Error adding LLM stroke to drawing widget '{widget_id}': {e}"
            )
            return {"success": False, "error": str(e)}
    ...
    def update_annotation(
        ...
        try:
            ...
            logger.error(
                f"Error updating annotation {index} on widget '{widget_id}': {e}"
            )
            return {"success": False, "error": str(e)}
